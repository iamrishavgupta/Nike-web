...
async function seed() {
  try {
    ...
    for (let i = 0; i < productNames.length; i++) {
      ...
      // Give each product several images so the gallery shows multiple views.
      const imageCount = 6;
      for (let k = 0; k < imageCount; k++) {
        const imgName = sourceImages[(i + k) % sourceImages.length];
        const src = join(sourceDir, imgName);
        const destName = `${insertedProduct.id}-${k}-${basename(imgName)}`;
