  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-center lg:hidden">
        <div className="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-orange">
          <Image src="/logo.svg" alt="Nike" width={22} height={22} />
        </div>
      </div>

      <p className="text-right text-caption text-dark-700">
        {mode === "sign-in" ? "Don’t have an account? " : "Already have an account? "}
        <Link href={mode === "sign-in" ? "/sign-up" : "/sign-in"} className="font-medium text-dark-900 underline">
      ...
      <div className="text-center">
        <h1 className="text-heading-3 text-dark-900">
          {mode === "sign-in" ? "Welcome back to Nike!" : "Join Nike Today!"}
        </h1>
      ...
      {error && (
        <div
          role="alert"
          className="rounded-xl border border-[color:var(--color-red)]/30 bg-[color:var(--color-red)]/5 px-4 py-3 text-caption text-[color:var(--color-red)]"
        >
          {error}
        </div>
      )}
      ...
      <form className="space-y-4" onSubmit={handleSubmit}>
        ...
        <div className="space-y-1">
          ...
          <div className="relative">
            ...
            <button
              type="button"
              className="absolute inset-y-0 right-0 flex items-center px-3 text-dark-700"
              onClick={() => setShow((v) => !v)}
        ...
        <button
          type="submit"
          disabled={loading}
          className="mt-2 w-full rounded-full bg-orange px-6 py-3 text-body-medium text-light-100 transition hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-orange disabled:opacity-60"
        >
          {loading
            ? mode === "sign-in"
              ? "Signing in…"
