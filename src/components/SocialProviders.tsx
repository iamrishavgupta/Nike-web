...
export default function SocialProviders({ variant = "sign-in" }: Props) {
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      // If configured correctly the browser redirects to Google and we never
      // reach here. If we do, the provider returned an error instead.
      if (res?.error) {
        setError("Google sign-in isn't configured yet. Add its OAuth keys to .env.local and restart the dev server.");
        setLoading(false);
      }
    } catch {
      setError("Google sign-in isn't configured yet. Add its OAuth keys to .env.local and restart the dev server.");
  ...
  return (
    <div className="space-y-3">
      ...
      >
        ...
        <span>{loading ? "Redirecting…" : `${actionLabel} with Google`}</span>
      </button>
