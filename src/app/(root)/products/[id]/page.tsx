...
export default async function ProductDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // All distinct images for the product, ordered (primary first), used by the
  // main gallery image and the thumbnail grid.
  ...
  const actionVariants = variants
    .map((v) => ({
      ...
      sizeSortOrder: v.size?.sortOrder ?? 0,
      inStock: v.inStock ?? 0,
    }))
    .filter((v) => v.sizeName);
  ...
  const subtitle =
    [product.gender?.label, product.category?.name].filter(Boolean).join(" · ") || undefined;
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      ...
      <section className="grid grid-cols-1 gap-10 lg:grid-cols-[1fr_480px]">
        {allImages.length > 0 && (
          <ProductGallery productId={product.id} image={allImages[0]} className="lg:sticky lg:top-6" />
        )}
        ...
        <div className="flex flex-col gap-6">
          ...
          <ProductThumbnails productId={product.id} image={allImages[0]} />
          <ProductActions
            productId={product.id}
            name={product.name}
            image={allImages[0]}
            variants={actionVariants}
            colorNames={galleryVariants.map((g) => g.color)}
          />
          ...
          <CollapsibleSection title="Shipping & Returns">
            <p>
              We offer free shipping on all orders over ₹8,300. For orders under ₹8,300, shipping is a flat rate of ₹830.
            </p>
            <p className="mt-2">
              This product is eligible for <span className="text-[color:var(--color-green)]">free shipping</span>.
            </p>
          </CollapsibleSection>
