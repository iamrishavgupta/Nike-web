"use client";

import { create } from "zustand";

type State = {
  selectedByProduct: Record<string, number>;
  setSelected: (productId: string, index: number) => void;
  getSelected: (productId: string, fallback?: number) => number;
  // Active gallery image index per product (shared between the main image and the thumbnail grid).
  imageByProduct: Record<string, number>;
  setImage: (productId: string, index: number) => void;
  getImage: (productId: string, fallback?: number) => number;
};

export const useVariantStore = create<State>((set, get) => ({
  selectedByProduct: {},
  setSelected: (productId, index) =>
    set((s) => ({
      selectedByProduct: { ...s.selectedByProduct, [productId]: index },
    })),
  getSelected: (productId, fallback = 0) => {
    const map = get().selectedByProduct;
    return map[productId] ?? fallback;
  },
  imageByProduct: {},
  setImage: (productId, index) =>
    set((s) => ({
      imageByProduct: { ...s.imageByProduct, [productId]: index },
    })),
  getImage: (productId, fallback = 0) => {
    const map = get().imageByProduct;
    return map[productId] ?? fallback;
  },
}));
